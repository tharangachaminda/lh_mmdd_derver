...
/**
 * Phase 1B Negative Numbers Ingestion Script
 * 
 * Ingests the Grade 8 Phase 1B NEGATIVE_NUMBERS dataset into OpenSearch
 * with proper vector embeddings and metadata enrichment.
 * 
 * Features:
 * - 15 high-quality negative number and integer questions
 * - Progressive difficulty: easy → medium → hard (6-6-3 distribution)
 * - Comprehensive integer operations and number line reasoning
 * - New Zealand curriculum Level 4 alignment
 * - Real-world application contexts
 * - Multi-step solution methodologies
 * - Five integer types: negative addition, negative subtraction, number line ordering, real-world applications, integer comparisons
 * 
...
// Configuration
const CONFIG = {
    questionsFile: join(__dirname, 'question_bank/grade8/grade8_negative_numbers_questions.json'),
...
async function enrichQuestion(question, metadata) {
    ...
    return {
        ...
        // Negative numbers specific metadata
        integerCategory: question.type || 'NEGATIVE_NUMBERS',
        integerSubcategory: determineIntegerSubcategory(question),
        hasStepByStepSolution: question.explanation?.includes('Step') || false,
        includesNegativeAddition: /\(-\d+\)\s*\+|\+.*negative|negative.*add/i.test(question.question + ' ' + question.explanation),
        includesNegativeSubtraction: /\(-\d+\)\s*-|-.*negative|negative.*subtract/i.test(question.question + ' ' + question.explanation),
        includesNumberLine: /number.*line|line.*number|position.*line/i.test(question.question + ' ' + question.explanation),
        includesRealWorldContext: /temperature|money|debt|floor|elevator|profit|loss/i.test(question.question + ' ' + question.explanation),
        includesComparison: /compare|greater|less|order|arrange/i.test(question.question + ' ' + question.explanation),
        ...
        primaryCategory: 'negative-numbers',
        ...
        properIntegerFormat: /\(-?\d+\)|negative|positive|\+|\-/i.test(question.question),
...
/**
 * Determine integer subcategory based on question content
 * @param {Object} question - Question object
 * @returns {string} Integer subcategory
 */
function determineIntegerSubcategory(question) {
    const questionText = question.question.toLowerCase();
    const explanationText = (question.explanation || '').toLowerCase();
    const combinedText = questionText + ' ' + explanationText;
    
    if (questionText.includes('calculate') && combinedText.includes('add')) {
        return 'negative-addition';
    } else if (questionText.includes('calculate') && combinedText.includes('subtract')) {
        return 'negative-subtraction';
    } else if (combinedText.includes('number line') || combinedText.includes('order')) {
        return 'number-line-ordering';
    } else if (combinedText.includes('temperature') || combinedText.includes('money') || combinedText.includes('real')) {
        return 'real-world-applications';
    } else if (combinedText.includes('compare') || combinedText.includes('greater') || combinedText.includes('less')) {
        return 'integer-comparisons';
    } else {
        return 'general-integer-operations';
    }
}
                ...
                body: {
                    ...
                    mappings: {
                        properties: {
                            ...
                            integerCategory: { type: 'keyword' },
                            integerSubcategory: { type: 'keyword' },
                            hasStepByStepSolution: { type: 'boolean' },
                            includesNegativeAddition: { type: 'boolean' },
                            includesNegativeSubtraction: { type: 'boolean' },
                            includesNumberLine: { type: 'boolean' },
                            ...
                            properIntegerFormat: { type: 'boolean' },
            ...
            try {
                ...
                operations.push({
                    index: {
                        ...
                        _id: `grade8_negative_numbers_${enrichedQuestion.id}`
                    }
                });
...
async function verifyIngestion() {
    try {
        ...
        console.log(`🔍 Verification: Found ${count} Phase 1B Negative Numbers questions in index`);
...
async function main() {
    ...
    console.log('➖ INGESTION PHASE: Grade 8 Phase 1B Negative Numbers');
    console.log('====================================================');
    ...
    try {
        ...
        console.log(`📚 Integer Types: ${metadata.integerTypes.length}`);
        console.log('');
        ...
        if (results.successCount === questions.length && verifiedCount === questions.length) {
            ...
            console.log('Phase 1B Negative Numbers successfully deployed to production!');
        } else {
...
export { main as ingestGrade8NegativeNumbers };