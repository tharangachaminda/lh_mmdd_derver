...
/**
 * Phase 1C Fraction, Decimal, Percentage Ingestion Script
 * 
 * Ingests the Grade 8 Phase 1C FRACTION_DECIMAL_PERCENTAGE dataset into OpenSearch
 * with proper vector embeddings and metadata enrichment.
 * 
 * Features:
 * - 25 high-quality fraction, decimal, and percentage questions
 * - Progressive difficulty: easy → medium → hard (10-10-5 distribution)
 * - Comprehensive rational number conversion patterns
 * - New Zealand curriculum Level 4 alignment
 * - Real-world application contexts (GST, shopping, Kiwi scenarios)
 * - Multi-step solution methodologies
 * - Five conversion types: fraction to decimal, decimal to percentage, percentage to fraction, real-world applications, benchmark conversions
 * 
 * @author Learning Hub Development Team
 * @version 1.0.0
 * @phase PHASE_1C
 * @topic FRACTION_DECIMAL_PERCENTAGE
 */
...
// Configuration
const CONFIG = {
    questionsFile: join(__dirname, 'question_bank/grade8/grade8_fraction_decimal_percentage_questions.json'),
...
async function enrichQuestion(question, metadata) {
    ...
    return {
        ...
        // Fraction/decimal/percentage specific metadata
        conversionCategory: question.type || 'FRACTION_DECIMAL_PERCENTAGE',
        conversionSubcategory: determineConversionSubcategory(question),
        hasStepByStepSolution: question.explanation?.includes('Step') || false,
        includesFractionToDecimal: /convert.*fraction.*decimal|1\/2.*0\.5|fraction.*divide/i.test(question.question + ' ' + question.explanation),
        includesDecimalToPercentage: /decimal.*percentage|0\.\d+.*%|multiply.*100/i.test(question.question + ' ' + question.explanation),
        includesPercentageToFraction: /percentage.*fraction|%.*fraction|convert.*%/i.test(question.question + ' ' + question.explanation),
        includesRealWorldContext: /GST|shop|price|discount|sale|kiwi|new zealand|tax/i.test(question.question + ' ' + question.explanation),
        includesBenchmarkFractions: /1\/2|1\/4|3\/4|1\/3|2\/3|1\/5|benchmark/i.test(question.question + ' ' + question.explanation),
        ...
        primaryCategory: 'fraction-decimal-percentage',
        ...
        properFractionFormat: /\d+\/\d+|\d+\.\d+|\d+%/i.test(question.question),
...
/**
 * Determine conversion subcategory based on question content
 * @param {Object} question - Question object
 * @returns {string} Conversion subcategory
 */
function determineConversionSubcategory(question) {
    const questionText = question.question.toLowerCase();
    const explanationText = (question.explanation || '').toLowerCase();
    const combinedText = questionText + ' ' + explanationText;
    
    if (questionText.includes('convert') && (combinedText.includes('fraction') && combinedText.includes('decimal'))) {
        return 'fraction-to-decimal';
    } else if (questionText.includes('convert') && (combinedText.includes('decimal') && combinedText.includes('percentage'))) {
        return 'decimal-to-percentage';
    } else if (questionText.includes('convert') && (combinedText.includes('percentage') && combinedText.includes('fraction'))) {
        return 'percentage-to-fraction';
    } else if (combinedText.includes('gst') || combinedText.includes('shop') || combinedText.includes('real') || combinedText.includes('price')) {
        return 'real-world-applications';
    } else if (combinedText.includes('benchmark') || combinedText.includes('1/2') || combinedText.includes('1/4')) {
        return 'benchmark-conversions';
    } else {
        return 'general-conversions';
    }
}
                ...
                body: {
                    ...
                    mappings: {
                        properties: {
                            ...
                            conversionCategory: { type: 'keyword' },
                            conversionSubcategory: { type: 'keyword' },
                            hasStepByStepSolution: { type: 'boolean' },
                            includesFractionToDecimal: { type: 'boolean' },
                            includesDecimalToPercentage: { type: 'boolean' },
                            includesPercentageToFraction: { type: 'boolean' },
                            includesRealWorldContext: { type: 'boolean' },
                            includesBenchmarkFractions: { type: 'boolean' },
                            hasMultiStepProcess: { type: 'boolean' },
                            hasEnhancedReasoning: { type: 'boolean' },
                            properFractionFormat: { type: 'boolean' },
            ...
            try {
                ...
                operations.push({
                    index: {
                        ...
                        _id: `grade8_fraction_decimal_percentage_${enrichedQuestion.id}`
                    }
                });
...
async function verifyIngestion() {
    try {
        ...
        console.log(`🔍 Verification: Found ${count} Phase 1C Fraction/Decimal/Percentage questions in index`);
...
async function main() {
    ...
    console.log('🔢 INGESTION PHASE: Grade 8 Phase 1C Fraction, Decimal, Percentage');
    console.log('===================================================================');
    ...
    try {
        ...
        console.log(`📚 Conversion Types: ${metadata.conversionTypes.length}`);
        console.log('');
        ...
        if (results.successCount === questions.length && verifiedCount === questions.length) {
            ...
            console.log('Phase 1C Fraction, Decimal, Percentage successfully deployed to production!');
        } else {
...
export { main as ingestGrade8FractionDecimalPercentage };