...
/**
 * Phase 1G Algebraic Manipulation Ingestion Script
 * 
 * Ingests the Grade 8 Phase 1G ALGEBRAIC_MANIPULATION dataset into OpenSearch
 * with proper vector embeddings and metadata enrichment.
 * 
 * Features:
 * - 35 high-quality algebraic manipulation questions
 * - Progressive difficulty: easy → medium → hard (14-14-7 distribution)
 ...
 * - Five algebraic types: expression expansion, like terms collection, variable substitution, formula applications, multi-step simplification
 * 
...
// Configuration
const CONFIG = {
    questionsFile: join(__dirname, 'question_bank/grade8/grade8_algebraic_manipulation_questions.json'),
...
async function enrichQuestion(question, metadata) {
    ...
    return {
        ...
        // Algebraic manipulation specific metadata
        algebraicCategory: question.questionType || 'ALGEBRAIC_MANIPULATION',
        ...
        includesExpansion: /expand|distributive|multiply.*bracket/i.test(question.question + ' ' + question.explanation),
        includesLikeTerms: /like.*terms|collect.*terms|combine.*terms/i.test(question.question + ' ' + question.explanation),
        includesSubstitution: /substitute|when.*=|let.*=/i.test(question.question + ' ' + question.explanation),
        ...
        primaryCategory: 'algebraic-manipulation',
        ...
        properAlgebraicFormat: /[a-z]\+|[a-z]\-|[a-z]\*|[0-9][a-z]|\([a-z]/i.test(question.question),
...
function determineAlgebraicSubcategory(question) {
    ...
    if (questionText.includes('expand') || combinedText.includes('distributive')) {
        return 'expression-expansion';
    } else if (combinedText.includes('like terms') || combinedText.includes('collect') || combinedText.includes('combine')) {
        return 'like-terms-collection';
    } else if (combinedText.includes('substitute') || combinedText.includes('when') || combinedText.includes('let')) {
        return 'variable-substitution';
    } else if (combinedText.includes('formula') || combinedText.includes('equation')) {
        return 'formula-application';
    } else if (explanationText.includes('step 2') || explanationText.includes('step 3')) {
        return 'multi-step-simplification';
    } else {
                ...
                body: {
                    ...
                    mappings: {
                        properties: {
                            ...
                            includesExpansion: { type: 'boolean' },
                            includesLikeTerms: { type: 'boolean' },
                            includesSubstitution: { type: 'boolean' },
            ...
            try {
                ...
                operations.push({
                    index: {
                        ...
                        _id: `grade8_algebraic_manipulation_${enrichedQuestion.id}`
                    }
                });
...
async function verifyIngestion() {
    try {
        ...
        console.log(`🔍 Verification: Found ${count} Phase 1G Algebraic Manipulation questions in index`);
...
async function main() {
    ...
    console.log('🧮 INGESTION PHASE: Grade 8 Phase 1G Algebraic Manipulation');
    console.log('============================================================');
    ...
    try {
        ...
        console.log(`📚 Algebraic Types: ${metadata.algebraicTypes.length}`);
        console.log('');
        ...
        if (results.successCount === questions.length && verifiedCount === questions.length) {
            ...
            console.log('Phase 1G Algebraic Manipulation successfully deployed to production!');
        } else {
...
export { main as ingestGrade8AlgebraicManipulation };