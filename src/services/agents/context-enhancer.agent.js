 ...
 */
export class ContextEnhancerAgent {
    constructor() {
        this.name = "ContextEnhancerAgent";
        this.description = "Enhances questions with engaging, age-appropriate real-world context";
    }
    /**
    ...
    async process(context) {
        ...
        if ("structuredPrompt" in context) {
            ...
            return (context.context.questions || [
                {
    ...
    async processLegacyContext(context) {
        try {
            ...
            for (let i = 0; i < context.questions.length; i++) {
                ...
                const enhancement = await this.enhanceQuestion(question, context.questionType, context.grade, context.difficulty);
     ...
     */
    async enhanceQuestion(question, questionType, grade, difficulty) {
        // Determine if question needs enhancement
        const needsEnhancement = this.shouldEnhanceQuestion(question.text, questionType);
        if (!needsEnhancement) {
            return {
        ...
        const enhancedText = this.generateEnhancedQuestion(question, questionType, grade, contextType);
        // Calculate engagement score
        const engagementScore = this.calculateEngagementScore(question.text, enhancedText, grade);
        return {
            originalText: question.text,
     ...
     */
    shouldEnhanceQuestion(questionText, questionType) {
        ...
        const hasStoryContext = storyIndicators.some((indicator) => questionText.toLowerCase().includes(indicator));
        if (hasStoryContext) {
            return false;
        }
     ...
     */
    selectContextType(questionType, grade) {
     ...
     */
    generateEnhancedQuestion(question, questionType, grade, contextType) {
        ...
        switch (contextType) {
            case "story":
                return this.generateStoryContext(question, questionType, grade, numbers);
            case "real-world":
                return this.generateRealWorldContext(question, questionType, grade, numbers);
            case "visual":
                return this.generateVisualContext(question, questionType, grade, numbers);
            default:
     ...
     */
    generateStoryContext(question, questionType, grade, numbers) {
        ...
        switch (questionType) {
            ...
            case QuestionType.MULTIPLICATION:
                const groups = ["bags", "boxes", "groups", "packs", "sets"][Math.floor(Math.random() * 5)];
                const objects = [
                    "apples",
                    "books",
                    "toys",
                ...
                return `${character} has ${a} ${groups}. Each ${groups.slice(0, -1)} contains ${b} ${objects}. How many ${objects} does ${character} have in total?`;
            case QuestionType.DIVISION:
                ...
                return `${character} has ${a} ${items2} to share equally among ${b} ${containers}. How many ${items2} will be in each ${containers.slice(0, -1)}?`;
            default:
                return question.text;
        }
    }
     ...
     */
    generateRealWorldContext(question, questionType, grade, numbers) {
        const [a, b] = numbers;
        switch (questionType) {
            case QuestionType.ADDITION:
                const scenarios = [
     ...
     */
    generateVisualContext(question, questionType, grade, numbers) {
     ...
     */
    calculateEngagementScore(originalText, enhancedText, grade) {
        ...
        const hasRelatableContent = relatableWords.some((word) => enhancedText.toLowerCase().includes(word));
