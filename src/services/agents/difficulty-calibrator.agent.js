 ...
 */
export class DifficultyCalibatorAgent {
    constructor() {
        this.name = "DifficultyCalibatorAgent";
        this.description = "Calibrates difficulty settings to ensure age-appropriate mathematical challenges";
    }
    /**
    ...
    async processStructuredPrompt(context) {
        try {
            console.log("📊 DifficultyCalibrator: Processing structured prompt...");
            ...
            console.log("✅ DifficultyCalibrator: Structured prompt processed (deterministic)");
            // Create structured response based on context
            return {
                difficulty_level: context.context.difficulty || "beginner",
                complexity_factors: {
                    numberRange: this.getSimpleNumberRange(context.context.grade || 5),
            ...
            console.error("❌ DifficultyCalibrator structured prompt failed:", error);
    ...
    async processLegacyContext(context) {
        try {
            ...
            const numberRange = this.calculateNumberRange(context.grade, context.difficulty, context.questionType);
            // Determine complexity and cognitive load
            const complexityAnalysis = this.analyzeComplexity(context.grade, context.difficulty, context.questionType);
            // Define allowed operations for this difficulty level
            const allowedOperations = this.getAllowedOperations(context.grade, context.questionType, context.difficulty);
            ...
            context.workflow.errors.push(`DifficultyCalibrator: ${errorMessage}`);
            return context;
        }
    }
    /**
    ...
    calculateNumberRange(grade, difficulty, questionType) {
        ...
        if (questionType === QuestionType.DIVISION) {
            ...
            adjustedRange.max = Math.min(adjustedRange.max, divisionSettings.maxDividend);
     ...
     */
    analyzeComplexity(grade, difficulty, questionType) {
     ...
     */
    getAllowedOperations(grade, questionType, difficulty) {
        ...
        if (grade <= 2) {
            operations = operations.filter((op) => !op.includes("double-digit") &&
