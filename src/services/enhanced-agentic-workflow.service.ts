...
interface WorkflowState {
    ...
    workflowMetrics: {
        startTime: number;
        agentTiming: Record<string, number>;
        retryCount: number;
...
export class LangGraphAgenticWorkflow {
     ...
     */
    private async runDifficultyCalibration(
        state: WorkflowState
     ...
     */
    private async runQuestionGeneration(
        state: WorkflowState
    ): Promise<Partial<WorkflowState>> {
        const startTime = Date.now();
        ...
        try {
            ...
            const formattedPrompt = await prompt.format({
                ...
                count: state.count,
                topic: state.topic,
                difficultySettings: JSON.stringify(
     ...
     */
    private async runQualityValidation(
        state: WorkflowState
    ): Promise<Partial<WorkflowState>> {
        const startTime = Date.now();
     ...
     */
    private async runContextEnhancement(
        state: WorkflowState
    ): Promise<Partial<WorkflowState>> {
        const startTime = Date.now();
        ...
        try {
            ...
            const formattedPrompt = await prompt.format({
                ...
                learningStyle: state.persona.learningStyle,
                validatedQuestions: JSON.stringify(
            ...
            const finalResult = {
                ...
                agentMetrics: {
                    ...
                    chainOfThought: true,
                    retryRate:
                        state.workflowMetrics.retryCount /
                        Math.max(
                            state.workflowMetrics.executionGraph.length,
                            1
                        ),
     ...
     */
    private shouldRetryGeneration(state: WorkflowState): boolean {
        if (
            state.error ||
            !state.generatedQuestions ||
     ...
     */
    private shouldRetryQuality(state: WorkflowState): boolean {
...
Example 2:
{
  ...
  "explanation": "8 + 3 = 11. Tom has 8 left plus the 3 he ate equals 11 total."
    ...
    async executeWorkflow(input: {
        ...
        try {
            console.log(
                "🟣 Executing Enhanced Workflow with LangChain Prompts..."
            );
            ...
            console.log(
                "📊 Step 1: Running difficulty calibration with structured prompts..."
            );
            ...
            console.log(
                "📝 Step 2: Running question generation with few-shot learning..."
            );
            ...
            console.log(
                "🔍 Step 3: Running quality validation with structured parsing..."
            );
            ...
            if (this.shouldRetryQuality(state)) {
                ...
                // Re-validate
                const revalidationResult = await this.runQualityValidation(
                    state
                );
            ...
            console.log(
                "🎨 Step 4: Running context enhancement with personalization..."
            );
            ...
            console.log(
                `⚡ Total execution time: ${
                    (state.workflowMetrics.agentTiming.total || 0) / 1000
                }s`
            );
            console.log(
                `🔄 Conditional routes used: ${state.workflowMetrics.conditionalRoutes.length}`
            );
