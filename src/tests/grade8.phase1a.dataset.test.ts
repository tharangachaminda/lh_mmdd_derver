...
describe("Grade 8 Phase 1A: Prime and Composite Numbers Dataset", () => {
    ...
    const expectedFilePath = path.join(
        __dirname,
        "../../question_bank/grade8/grade8_prime_composite_numbers_questions.json"
    );
    ...
    describe("Dataset Metadata Validation", () => {
        test("should have correct NZ curriculum-aligned metadata structure", () => {
            ...
            expect(dataset.metadata.datasetId).toBe(
                "grade8-prime-composite-numbers-2025"
            );
            expect(dataset.metadata.datasetName).toBe(
                "Grade 8 Prime and Composite Numbers Questions"
            );
            ...
            expect(dataset.metadata.curriculumSubtopic).toBe(
                "Prime numbers, composite numbers, cube numbers, and powers of 10"
            );
    ...
    describe("NZ Curriculum Content Validation", () => {
        test("should include prime number identification questions", () => {
            const primeQuestions = dataset.questions.filter(
                (q) =>
        ...
        test("should include composite number analysis questions", () => {
            const compositeQuestions = dataset.questions.filter(
                (q) =>
        ...
        test("should include cube number questions", () => {
            const cubeQuestions = dataset.questions.filter(
                (q) =>
        ...
        test("should include prime factorization questions", () => {
            const factorQuestions = dataset.questions.filter(
                (q) =>
            ...
            dataset.questions.forEach((question) => {
                ...
                expect(
                    question.keywords.some((k) =>
                        ["prime", "composite", "cube", "factor", "number"].some(
        ...
        test("should have unique question IDs following Grade 8 convention", () => {
            ...
            dataset.questions.forEach((question) => {
                expect(question.id).toMatch(
                    /^g8-PRIME_COMPOSITE_NUMBERS-(easy|medium|hard)-\d+$/
                );
            });
        });
    ...
    describe("Educational Value Validation", () => {
        test("should progress logically from easy to hard concepts", () => {
            ...
            const basicConcepts = easyQuestions.filter(
                (q) =>
            ...
            const complexConcepts = hardQuestions.filter(
                (q) =>
