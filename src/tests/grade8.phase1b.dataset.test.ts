...
describe("Grade 8 Phase 1B: Negative Numbers Dataset", () => {
    ...
    const expectedFilePath = path.join(
        __dirname,
        "../../question_bank/grade8/grade8_negative_numbers_questions.json"
    );
    ...
    describe("Dataset Metadata Validation", () => {
        test("should have correct NZ curriculum-aligned metadata structure", () => {
            ...
            expect(dataset.metadata.datasetId).toBe(
                "grade8-negative-numbers-2025"
            );
            expect(dataset.metadata.datasetName).toBe(
                "Grade 8 Negative Numbers Questions"
            );
            ...
            expect(dataset.metadata.curriculumSubtopic).toBe(
                "Operations with negative numbers and integer understanding"
            );
    ...
    describe("NZ Curriculum Content Validation", () => {
        test("should include negative number addition questions", () => {
            const additionQuestions = dataset.questions.filter(
                (q) =>
                    (q.question.toLowerCase().includes("add") ||
                        q.question.includes("+")) &&
                    (q.question.includes("-") ||
        ...
        test("should include negative number subtraction questions", () => {
            const subtractionQuestions = dataset.questions.filter(
                (q) =>
                    (q.question.toLowerCase().includes("subtract") ||
                        q.question.includes("-")) &&
                    q.explanation.toLowerCase().includes("negative")
            );
        ...
        test("should include number line and ordering questions", () => {
            const orderingQuestions = dataset.questions.filter(
                (q) =>
        ...
        test("should include real-world applications with negative numbers", () => {
            const applicationQuestions = dataset.questions.filter(
                (q) =>
    ...
    describe("Question Quality Validation", () => {
        ...
        test("should have unique question IDs following Grade 8 convention", () => {
            ...
            dataset.questions.forEach((question) => {
                expect(question.id).toMatch(
                    /^g8-NEGATIVE_NUMBERS-(easy|medium|hard)-\d+$/
                );
            });
        });
        ...
        test("should have vector-ready content embedding", () => {
            dataset.questions.forEach((question) => {
                ...
                expect(
                    question.contentForEmbedding.length
                ).toBeGreaterThanOrEqual(20);
                expect(question.contentForEmbedding.length).toBeLessThanOrEqual(
                    200
                );
            });
        });
    ...
    describe("Educational Value Validation", () => {
        test("should progress logically from easy to hard concepts", () => {
            ...
            const basicOperations = easyQuestions.filter(
                (q) =>
                    q.question.includes("+") ||
                    q.question.includes("-") ||
                    q.question.toLowerCase().includes("add") ||
            ...
            const complexProblems = hardQuestions.filter(
                (q) =>
        ...
        test("should demonstrate proper mathematical notation and language", () => {
            ...
            const notationQuestions = dataset.questions.filter(
                (q) =>
                    q.question.includes("-") ||
                    q.question.includes("+") ||
